...
export const auraContractAddress = process.env.NEXT_PUBLIC_AURA_CONTRACT_ADDRESS || '';
...
export async function getTokenBalance(userAddress: string): Promise<number> {
  if (!userAddress || !auraContractAddress) return 0;
  try {
    const operation = Operation.invokeContractFunction({
      contract: auraContractAddress,
      function: 'balance',
...
export async function initiateVesting(
  ...
  const operation = Operation.invokeContractFunction({
    ...
    args: [
      ...
      new Address(auraContractAddress).toScVal(),
